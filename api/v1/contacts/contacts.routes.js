...
module.exports.initRoutes = (app) => {
    ...
    //create new contact
    app.post('/api/v1/contacts/:user', (req,res,next) => {
        res.status(500).json({data:`eventually I'll put contact info into ${req.params.id}`});
    });

    //read ALL contacts
    app.get('/api/v1/contacts/:user', (req,res,next) => {
        res.status(200).json({data: `get contacts belonging to.... ${req.params.user}`});
    });

    //read ONE contact
    app.get('/api/v1/contacts/:user/:id' , (req, res, next) => {
        res.status(200).json({data: `get specific contact. user it belonds to ${req.params.user}, looking for ${req.params.id}`});
    });

    //update contact
    app.put('/api/v1/contacts/:user/:id', (req,res,next) =>{
